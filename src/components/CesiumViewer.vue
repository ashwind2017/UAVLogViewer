...
export default {
    ...
    methods: {
        async asyncSetup () {
            if (this.viewer == null) {
                // Always use offline mode if no Cesium token is available
                if (this.state.isOnline && Ion.defaultAccessToken && Ion.defaultAccessToken.length > 0) {
                    console.log('Using online Cesium viewer with token')
                    this.viewer = this.createViewer(true)
                    if (this.state.vehicle !== 'boat') {
                        this.viewer.terrainProvider = await createWorldTerrainAsync()
                    }
                } else {
                    console.log('Using offline Cesium viewer (no token)')
