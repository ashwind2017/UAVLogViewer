...
export default {
  ...
  watch: {
    animationSpeed(newSpeed) {
      ...
      // Adjust start time to maintain current position at new speed
      if (this.isAnimating && this.animationCursor) {
        this.animationStartTime = Date.now() - (this.animationProgress * this.totalFlightTime * 1000 / newSpeed)
      }
  ...
  methods: {
    ...
    toggleAnimation() {
      ...
      if (this.isAnimating) {
        ...
        if (this.animationCursor) {
          ...
          this.animationStartTime = Date.now() - (this.animationProgress * this.totalFlightTime * 1000 / this.animationSpeed)
          this.startFlightAnimation()
        }
      } else {
        // Pause animation but keep cursor visible
        if (this.animationCursor) {
          this.animationCursor.visible = true // Keep visible when paused
        }
      }
    },
    ...
    startFlightAnimation() {
      ...
      this.animationProgress = Math.min(elapsedTime / this.totalFlightTime, 1)
      ...
      const exactIndex = this.animationProgress * (this.clickablePoints.length - 1)
      ...
      if (pointIndex < this.clickablePoints.length && nextIndex < this.clickablePoints.length) {
        ...
        const localProgress = exactIndex - pointIndex
